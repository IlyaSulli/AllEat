...
class _CartState extends State<Cart> {
  ...
  void _updateCartDetails(
    profileid,
    price,
  ) {
    for (int l = 0; l < finalCartData.length; l++) {
      if (finalCartData[l][0].toString() == profileid.toString()) {
        setState(() {
          finalCartData[l][6] = (finalCartData[l][6] * 100 + price * 100) / 100;
        });
      }
    }
  }
                ...
                Column(
                  children: [
                    ...
                    ListView.builder(
                        ...
                        shrinkWrap: true,
                        itemCount: availableProfiles.length + 1, //For each profile
                        itemBuilder: (context, avPrIndex) {
                          if (avPrIndex == availableProfiles.length) {
                            while (compeletedGettingCartData == false) {
                              return Row(
                                  mainAxisAlignment: MainAxisAlignment.center,
                                  children: [CircularProgressIndicator(color: Theme.of(context).primaryColor)]);
                            }
                            return Text(finalCartData.toString());
                          } else {
