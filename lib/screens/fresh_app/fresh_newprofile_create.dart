...
class FreshRegisterProfilePageState extends State<FreshRegisterProfilePage> {
  ...
  Future<bool> sendData() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (data["error"]) {
          ...
          await SQLiteLocalDB.createProfile(
              ...
              email.text,
              data[
                  "hash"]); //On success, create new profile entry on localprofiles table within the app
          ...
          await SQLiteLocalDB.setSelected(profileInfoTemp[0][
              "id"]); //Get the new registered user and set them to be the selected profile using their stored id
          ...
          setState(() {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                  content: Text(
                      'Profile Successfully Created.')), //Show success banner at bottom temporarily
            );
        ...
        //If the app was able to send the data but there was an issue with the database
        ...
        setState(() {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text(
                    "Error ${res.statusCode}: Failed to connect to server.")), //Display the error with the generated error code from the server
          );
      ...
      setState(() {
        //If there was an error sending the data causing an app error, catch the error
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'FAILED: $e')), //Convert to error message and display to the user to prevent crashing
        );
        error = true;
        msg = "Connection to database failed.";
        sending = false;
                ...
                onPressed: () {
                  if (_formKey.currentState!.validate()) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                          content: Text(
                              'Creating profile...')), //Display creating profile and start Future to send data to the server
                    );
                    setState(() {
